...
- 设置页新增「版本」页面：显示当前 Build 版本号（从后端实时获取）与更新日志。
- 新增 APIMart 中转站 Seedance 2.0 系列（doubao-seedance-2.0 / -fast / -mini）视频生成预设与调用规则，与火山官方路径完全隔离、独立共存。
- 外观设置新增「星辰」「流光束」低资源动态背景，支持流光束配色自定义与「减弱动效」（限制 30 FPS 并降速，不直接暂停动画）。

### 优化

- 「AI 配置」更名为「设置」，设置页全面统一为玻璃卡片设计（Liquid Glass 模式下避免实色色块），SD2 资产管理板块重构为纵向分区布局。
- 移除旧的 Silk / Side Rays WebGL 背景（高显存占用），改用 CSS / Canvas2D 实现的低占用动态背景；外观弹窗大小自适应内容。
- 业务场景板块修复 .page-header 类名与全局顶部栏冲突导致的早期背景残留。

### 修复

- 修复 APIMart 路径未注入声线绑定关系文本的问题：带声线绑定的角色引用现在会生成「图片N中的角色使用音频M的声线参考」关系，与火山官方路径保持一致；临时静音的素材按普通图片引用方式输出。

## [1.4.25] - 2026-07-21

### 优化

- 自由创作生成按钮的文字状态增加淡出、轻微模糊与淡入过渡；任务提交后的灰色冷却态及三秒后的恢复增加颜色、亮度、透明度和阴影渐变。

## [1.4.24] - 2026-07-21

### 优化

- 扩大自由创作生成按钮的确认态表面高光，减慢扫过速度并缩短轮次间隔，使光线移动更连续；“再次点击确认生成”增加中性白灰文字发光。

## [1.4.23] - 2026-07-21

### 优化

- 移除生成按钮确认态的外层 StarBorder，改为按钮表面内部的窄幅高光扫过效果；高光位于文字下方，并在每轮扫过后留出停顿，不再覆盖整块按钮。

## [1.4.22] - 2026-07-21

### 优化

- 自由创作生成按钮的二次确认态改为 StarBorder 边缘流光：上下两束中性白灰光沿边框相向移动，移除原呼吸阴影效果，并兼容“减少动态效果”系统设置。

## [1.4.21] - 2026-07-21

### 优化

- 移除自由创作顶部多余的 Seedance 2.0 标签。
- 生成按钮改为二次确认：首次点击进入呼吸高光确认态，八秒内再次点击才提交；任务创建后按钮锁定三秒，防止连续点击重复创建任务。
- 提交后的前端任务轮询与按钮状态解耦，按钮冷却结束后恢复正常，生成进度继续在后台更新。

## [1.4.20] - 2026-07-21

### 新增

- 自由创作新增联网搜索开关，默认关闭；开启时按火山方舟官方格式提交 `tools: [{ "type": "web_search" }]`，并随视频版本快照保存。

### 修复

- Seedance 2.0 请求固定开启有声视频、固定关闭水印，不再接受前端参数或旧版本快照覆盖。
- 明确区分编辑器中的 `@图片N` 交互标记与接口实际接收的“图片N”提示词引用，数据库仍保存可编辑的原始标记。

## [1.4.19] - 2026-07-21

### 新增

- 角色、场景、道具素材新增图片版本记录；重新上传自动建立新版本，旧版与新版分别保存 TOS URL、同步时间和七天有效期。
- 素材编辑页可查看缩略图版本历史并快速切换，切回旧版时同步恢复该版自己的 TOS 状态。

### 修复

- Seedance 2.0 多模态请求体按方舟当前 Contents Generation 格式统一：图片使用 `reference_image`、音频使用 `reference_audio`，并启用 `generate_audio`。
- 发送前将编辑器的 `@图片N` 标记规范化为官方提示词中的“图片N”，角色绑定音频自动生成“图片N—音频M”声线关系。
- 移除全能参考请求中多余的旧式 `task_type` 字段，并新增请求体与素材版本回归测试。

## [1.4.18] - 2026-07-20

### 新增

- 全局与项目素材支持手动或引用时自动同步到 TOS，并持久化对象地址、同步时间和七天有效期；素材卡与引用列表显示 TOS 状态。
- 自由创作支持最多 3 条、总计不超过 15 秒的统一音频参考；角色绑定声音会随角色图片成对提交，并可针对当前镜头临时静音。

### 修复

- 修复全局角色声音绑定写入成功后，刷新素材库却显示未绑定的问题。
- 合并自由创作中的“音频”和“角色声音”入口，避免同一素材库重复展示。

## [1.4.17] - 2026-07-20

### 优化

- 任务列表端点与取消/删除端点的常驻说明改为与其他配置字段一致的问号悬浮提示，减少表单纵向占用。

## [1.4.16] - 2026-07-20

### 优化

- 火山视频配置不再同时展示四个可编辑端点与重复的最终 URL 预览卡片，只保留后端实际读取的四个可编辑端点字段。
- Base URL 与相对端点仍在请求时只拼接一次；填写完整 http(s) URL 时仍原样执行。

## [1.4.15] - 2026-07-20

### 修复

- 火山视频的 Base URL 不再被强制重写为标准 `/api/v3`；Plan 专属 URL、标准 URL 与自定义 URL 均按用户保存值执行。
- 提交、查询、任务列表、取消/删除四个端点均支持可编辑相对路径或完整 URL；相对路径只拼接一次 Base URL，完整 URL 原样使用。
- 火山视频连接测试改用可配置的只读任务列表接口，不再借用 `/chat/completions` 判断视频 Key。
- 移除 Seedance 模型 ID 的运行时别名替换，用户填写的模型 ID 与 API Key 均原样提交，让上游返回真实错误。

### 验证

- 后端 66 项回归测试全部通过，包含 Plan/标准 Base URL、完整端点、查询、列表与 DELETE 取消请求；网页生产构建通过。

## [1.4.14] - 2026-07-20

### 修复

- TOS 连接测试现在解析 `x-tos-ec` 和 Request ID；对官方错误 `0002-00000020` 直接提示“AccessKey ID 不存在”，不再显示“未返回可识别的错误信息”。
- TOS 配置页明确标注必须使用火山引擎 IAM 密钥管理中成对的 AccessKey ID / Secret Access Key，不能使用方舟 API Key 或其他 Key ID。

## [1.4.13] - 2026-07-20

### 修复

- 修复 macOS 本地存在 `HTTP_PROXY/HTTPS_PROXY=http://...` 时，TOS SDK 将 HTTPS Endpoint 错误交给 HTTP transport，并提示 `Protocol "http:" not supported. Expected "https:"` 的问题。TOS 客户端现在始终使用 HTTPS 直连，不影响其他 AI 供应商的代理设置。
- TOS 连接测试限定为单次 `HeadBucket`，禁用 SDK 自动重试；前后端同时阻止并发点击，并将超时限制为 12 秒。正式上传同样每张只执行一次 `PutObject`，20 秒内失败即回落第三方图床。

### 验证

- 增加 HTTPS/代理回归测试；后端 61 项、前端 13 项测试全部通过，网页生产构建通过。

## [1.4.12] - 2026-07-20

### 新增

- **TOS 独立存储配置**：AI 配置页新增“存储”工作台，支持 AccessKey、Endpoint、Region、Bucket、对象前缀、STS Token、自定义公网域名与预签名有效期配置。
- **TOS 连接测试**：使用 HeadBucket 验证权限和 Bucket 可达性，不创建测试对象。

### 优化

- 本地参考图需要转为公网地址时，统一执行“TOS 优先 → 现有第三方图床回退”链路；TOS 未启用、配置不完整或上传失败都不会中断现有生成流程。
- 私有 Bucket 默认生成预签名 GET URL；配置公开域名或 CDN 后直接生成稳定对象地址。
- 密钥不回传前端，编辑时留空保留原密钥；支持环境变量覆盖页面配置。

### 验证

- 增加 TOS 密钥脱敏、PutObject、预签名 URL、HeadBucket 以及 TOS 成功/失败分支的回归测试。
- 通过 1440×900 Liquid Glass 桌面视口检查，存储标签与完整配置界面可正常切换。

## [1.4.11] - 2026-07-20

### 新增

- **自由创作 `@` 素材引用**：全能参考提示词改用光标跟随的素材选择器，支持鼠标、上下键与回车选择；编辑器显示素材名称，持久化和 Seedance 请求统一输出为 `@图片N`。

### 优化

- 参考图片严格按自由创作素材栏的排列顺序构造方舟 `content` 数组，第三方公网图床 URL 保持直传；本地素材继续按现有配置选择 base64 或第三方图床转译，为后续可选 TOS 存储保留扩展边界。
- 删除某张参考图时同步移除其提示词标签，并将后续 `@图片N` 自动前移，避免素材与文本绑定错位。

### 修复

- 修复原“添加参考图片文字”只插入占位文本、没有建立真实图片引用的问题。
- 修复空参考地址或单张图片解析失败时静默压缩图片数组、导致后续 `@图片N` 指向错误素材的问题；现在会在任务提交前明确阻止或中止。

### 验证

- 三张参考图按 `@图片1、@图片3、@图片2` 乱序出现在提示词时，仍保持与请求图片槽位一一对应；前端 13 项、后端 54 项测试全部通过。

## [1.4.10] - 2026-07-20

### 新增

- **Seedance 2.0 Mini**：火山视频预设增加 `doubao-seedance-2-0-mini-260615`，沿用 Seedance 2.0 系列的多模态参考与 4–15 秒参数规则。
- **火山视频任务管理**：AI 配置页可查询火山方舟视频任务列表，并通过官方任务资源接口取消排队任务或删除任务记录。
- **自由创作视频生命周期**：预览区新增下载、取消与复制操作；复制会生成一个带完整提示词、参考素材和生成参数快照的新视频条目。

### 优化

- **AI 配置工作台**：五个配置分类迁入全局顶部导航，移除页面最外层卡片限制，内容按全宽桌面工作区展示并采用轻量叠化上浮过渡。
- **入口范围收敛**：外观与 AI 配置按钮仅在项目空间主页出现，素材库和各项目内页面不再重复展示。
- **火山地址兼容**：历史配置中的错误官方路径会自动归一到 `https://ark.cn-beijing.volces.com/api/v3`。

### 修复

- 自由创作重新选中历史视频时恢复其提示词、参考素材、模式、模型、时长、比例、分辨率与声音引用，避免视频与创作输入脱节。
- 火山任务管理弹窗遵循顶部导航安全区和视口最大高度，避免正文或操作区被裁切。

## [1.4.9] - 2026-07-20

### 优化

- **AI 配置独立页面**：全局顶部栏与创作流程内的 AI 配置入口统一跳转至独立路由，并携带来源地址以便原路返回；配置标签和数据表在固定桌面工作区内滚动，不再挤压主界面。
- **配置弹窗安全区**：添加/编辑、接口说明、一键配置、素材列表、连接测试与批量换 Key 等功能窗口统一挂载到页面最外层，限制在顶部导航下方和视口安全高度内，超长内容仅在弹窗正文滚动。

### 修复

- 修复 AI 配置主弹窗内继续打开功能弹窗时，二级窗口被父级边界裁切、底部内容显示不全的问题。
- 修复从自由创作、剧集等不同层级进入 AI 配置后无法准确返回原页面的问题。

## [1.4.8] - 2026-07-20

### 优化

- **顶部栏材质统一**：项目、进入制作、返回剧集、画布/列表模式、画布工具组、外观与 AI 配置统一为 38px 高的透明静止态按钮，仅在悬停时浮现轻量玻璃膜，并保留由内容决定的自然宽度。
- **素材库可读性恢复**：素材卡片与添加操作使用独立的局部表面透明度，不再被顶部导航或通用轻量卡片规则联动覆盖；简约外观继续使用不透明实体材质。

### 修复

- 修复顶部栏主按钮、普通按钮、文本按钮与 Canvas 小按钮高度、透明度和交互反馈不一致的问题。
- 修复通用卡片与按钮规则将素材库卡片压至过低透明度、造成文字可读性下降的问题。

### 发行

- 网页版、macOS arm64 桌面版和 Windows x64 完整 BuildKit 统一提升至 **1.4.8**。

## [1.4.7] - 2026-07-20

### 优化

- **全局背景连续性**：动态背景固定在应用根层并跨路由复用，项目空间、素材库、剧集与自由创作页面切换时不再重新闪烁或跟随页面滚动。
- **桌面动效统一**：按桌面横屏交互重新校准导航、侧栏、标题、工具栏和内容卡片的切换时序，减少页面切换时的重排与元素重叠。
- **素材库切换动画重写**：移除会与 CSS Grid 争夺位置的列表 FLIP/离场动画；旧卡即时卸载，新卡在最终网格位置执行 8px 上浮与 30ms 逐项错峰，角色、场景、道具、音频切换不再出现卡片放大或漂移。

### 修复

- 修复素材卡片离场时脱离网格、瞬间扩展覆盖内容区的问题。
- 修复频繁切换素材标签时卡片入场与出场同时修改 `transform`，导致位置乱飘的问题。
- 修复部分层级页面返回项目空间时背景重载、卡片重叠和侧栏瞬时矫正的问题。

### 发行

- 网页版与 macOS arm64 桌面版统一提升至 **1.4.7**。
- 提供面向 Windows x64 的完整 **1.4.7 BuildKit**，用于从现有 1.4.4 工程重新构建，而不是叠加多份历史补丁。
