...
function stopWindowsAppProcesses() {
  ...
  const names = ['CineGen.exe', '本地短剧助手.exe', 'LocalMiniDrama.exe'];
  ...
  try {
    const ps = [
      ...
      "Where-Object { $_.ExecutablePath -like '*CineGen*' -or $_.ExecutablePath -like '*LocalMiniDrama*' -or $_.ExecutablePath -like '*win-unpacked*' -or $_.Name -like 'CineGen*' -or $_.Name -like 'LocalMiniDrama*' } |",
...
function removeDir(dir) {
  ...
  try {
    ...
    try {
      ...
      log('[clean] Close running CineGen.exe, close Explorer on release/win-unpacked, then retry.');
